...
class OAuthService {
  constructor(private client: ReturnType<typeof axios.create>) {
    const standaloneLocalMode = Boolean(process.env.LOCAL_ADMIN_TOKEN && !ENV.oAuthServerUrl);
    if (standaloneLocalMode) {
      console.log("[Auth] Standalone VPS mode: managed OAuth is disabled; the local administrator bootstrap is active.");
    } else {
      console.log("[OAuth] Initialized with baseURL:", ENV.oAuthServerUrl);
    }
    if (!ENV.oAuthServerUrl && !standaloneLocalMode) {
      console.error(
