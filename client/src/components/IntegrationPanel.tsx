              ...
              {isOpen && (
                <div className="mt-4 space-y-3 border-t border-dashed border-gray-200 pt-4">
                  ...
                  {service.requiredSecrets.map(keyName => (
                    <div key={keyName}>
                      <label className="text-xs font-extrabold text-gray-700">{keyName}</label>
                      <div className="mt-1.5 flex gap-2">
                        ...
                        <Button aria-label={`Save ${keyName}`} disabled={!drafts[keyName]?.trim() || saveSecret.isPending} onClick={() => saveSecret.mutate({ restaurantId: rid, provider: service.provider as Provider, keyName, value: drafts[keyName] })} className="h-10 shrink-0 rounded-xl bg-[#c84630] px-3 font-extrabold hover:bg-[#b03a28]">
