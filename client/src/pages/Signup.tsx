      ...
      <div className="min-h-screen bg-[#F8F9FA] flex items-center justify-center p-4">
        <div className="w-full max-w-md space-y-8">
          <div className="text-center space-y-2">
            ...
            <h1 className="text-3xl font-bold text-gray-900 font-extrabold tracking-tight">
              Welcome Back
            </h1>
            <p className="text-gray-600">Sign in to manage your restaurant</p>
          </div>

          <form onSubmit={handleLogin} className="space-y-4 bg-white rounded-2xl p-8 shadow-sm border border-gray-200">
          ...
          <p className="text-center text-sm text-gray-600">
            ...
            <button onClick={() => setIsLogin(false)} className="text-gray-900 font-semibold hover:underline">
              Create one
            </button>
          </p>
        </div>
    ...
    <div className="min-h-screen bg-[#F8F9FA] flex items-center justify-center p-4">
      <div className="w-full max-w-md space-y-8">
        <div className="text-center space-y-2">
          ...
          <p className="text-gray-600">Create your branded ordering page in minutes</p>
        </div>

        <div className="flex gap-2 mb-6">
          <div className={`flex-1 h-1.5 rounded-full ${step === "account" ? "bg-gray-900" : "bg-gray-900/30"}`} />
          <div className={`flex-1 h-1.5 rounded-full ${step === "restaurant" ? "bg-gray-900" : "bg-gray-900/30"}`} />
        </div>

        {step === "account" ? (
          <form onSubmit={handleAccountNext} className="space-y-4 bg-white rounded-2xl p-8 shadow-sm border border-gray-200">
            <div className="space-y-2">
          ...
          <form onSubmit={handleRegister} className="space-y-4 bg-white rounded-2xl p-8 shadow-sm border border-gray-200">
            <div className="space-y-2">
            ...
            {form.restaurantSlug && (
              <p className="text-xs text-gray-600 -mt-2">
                Your storefront URL: <span className="font-mono text-gray-900">/{form.restaurantSlug}</span>
              </p>
            )}
            <div className="space-y-2">
            ...
            <div className="flex gap-3 pt-2">
              ...
              <Button type="submit" className="flex-1 bg-gray-900 hover:bg-gray-800 text-white" disabled={isLoading}>
                {isLoading ? <LoaderCircle className="w-4 h-4 animate-spin mr-2" /> : null}
                Create Restaurant
              </Button>
            </div>
        ...
        <p className="text-center text-sm text-gray-600">
          ...
          <button onClick={() => setIsLogin(true)} className="text-gray-900 font-semibold hover:underline">
            Sign in
          </button>
        </p>
      </div>
