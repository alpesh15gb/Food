  ...
  return (
    <main className="grid min-h-screen place-items-center bg-[#F8F9FA] p-5">
      <section className="max-w-md rounded-[2rem] bg-white p-8 text-center shadow-xl shadow-gray-200/10">
        ...
        <h1 className="font-extrabold tracking-tight mt-5 text-3xl text-gray-900">
        ...
        {localAccess.data ? (
          ...
          >
            ...
            {accessError && (
              <p
                role="alert"
                className="mt-3 rounded-xl bg-red-50 px-3 py-2.5 text-sm font-bold leading-relaxed text-red-600"
              >
                {accessError}
              </p>
            )}
          ...
          >
            ...
            <label className="mt-3 block text-sm font-extrabold text-gray-700">
              Password
              <Input
                value={password}
      ...
      <div className="mx-auto max-w-7xl">
        <div className="mb-6 flex items-center gap-3">
          ...
          <div>
            ...
            <h1 className="font-extrabold tracking-tight text-3xl">Import your live menu</h1>
          </div>
        </div>
      ...
      <header className="border-b border-gray-200 bg-white px-5 py-5 lg:px-8">
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-4">
          <div>
            ...
            <h1 className="font-extrabold tracking-tight mt-1 text-3xl">
              {section === "overview"
                ? "Today at a glance"
                : section === "orders"
                ? "Order queue"
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        {cards.map((card) => (
          ...
          >
            ...
            <p className="font-extrabold tracking-tight mt-1 text-3xl">{card.value}</p>
      ...
      <section className="grid gap-5 lg:grid-cols-[1.35fr_0.65fr]">
        <article className="rounded-2xl bg-gray-900 p-6 text-white shadow-sm">
          <div className="flex items-center gap-3">
            ...
            />
            <p className="text-[10px] font-extrabold uppercase tracking-[0.16em] text-gray-300">
              Restaurant status
            </p>
          </div>
          <h2 className="font-extrabold tracking-tight mt-2 text-3xl">
          ...
          <div className="mt-6 flex items-center gap-2 text-xs font-extrabold text-gray-300">
        ...
        <article className="rounded-2xl bg-white p-5 shadow-sm">
          ...
          <h2 className="font-extrabold tracking-tight mt-1 text-2xl">Kitchen controls</h2>
          <div className="mt-5 space-y-3">
            <button
...
function MenuPanel({
  ...
  return (
    <div className="grid gap-5 xl:grid-cols-[0.72fr_1.28fr]">
      {/* Add Item Form */}
      <aside className="rounded-2xl bg-gray-900 p-5 text-white shadow-sm">
        <p className="text-[10px] font-extrabold uppercase tracking-[0.16em] text-gray-300">
          Add a dish
        </p>
        <h2 className="font-extrabold tracking-tight mt-2 text-3xl">Make the menu yours</h2>
            ...
            <article key={item.id} className="p-4">
              {editingId === item.id ? (
                ...
                <div className="space-y-3 rounded-xl border border-gray-200 bg-gray-50 p-4">
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 space-y-2">
                      <Input value={editForm.name ?? ""} onChange={(e) => setEditForm({ ...editForm, name: e.target.value })} placeholder="Name" className="h-9 rounded-lg border-gray-200 text-sm" />
                      <textarea value={editForm.description ?? ""} onChange={(e) => setEditForm({ ...editForm, description: e.target.value })} placeholder="Description" rows={2} className="w-full rounded-lg border border-gray-200 px-3 py-1.5 text-sm resize-none" />
                    </div>
                  </div>
                  <div className="grid grid-cols-3 gap-2">
                    <Input value={editForm.pricePaise != null ? (editForm.pricePaise / 100).toFixed(2) : ""} onChange={(e) => setEditForm({ ...editForm, pricePaise: Math.round(Number(e.target.value) * 100) })} placeholder="Price ₹" inputMode="numeric" className="h-9 rounded-lg border-gray-200 text-sm" />
                    <Input value={editForm.offerPricePaise != null ? (editForm.offerPricePaise / 100).toFixed(2) : ""} onChange={(e) => setEditForm({ ...editForm, offerPricePaise: e.target.value ? Math.round(Number(e.target.value) * 100) : null })} placeholder="Offer ₹" inputMode="numeric" className="h-9 rounded-lg border-gray-200 text-sm" />
                    <select value={editForm.dietaryType ?? "veg"} onChange={(e) => setEditForm({ ...editForm, dietaryType: e.target.value })} className="h-9 rounded-lg border border-gray-200 px-2 text-sm">
                ...
                <div className="flex items-center gap-4">
                  ...
                  {item.imageUrl ? (
                    <img src={item.imageUrl} alt="" className="h-10 w-10 shrink-0 rounded-lg object-cover border border-gray-200" />
                  ) : (
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <select
                      ...
                      className="rounded-lg border border-gray-200 bg-white px-2 py-2 text-xs font-extrabold"
                    >
  ...
  return (
    <div className="grid gap-5 lg:grid-cols-[0.7fr_1.3fr]">
      ...
      <aside className="rounded-2xl bg-gray-900 p-5 text-white shadow-sm">
        ...
        <h2 className="font-extrabold tracking-tight mt-2 text-3xl">Organise your menu</h2>
        <div className="mt-6 space-y-3">
          <Input
            value={newName}
      ...
      <section className="overflow-hidden rounded-2xl bg-white shadow-sm">
        <div className="border-b border-gray-200 p-5">
          ...
          <h2 className="font-extrabold tracking-tight mt-1 text-2xl">Visibility & availability</h2>
        ...
        <div className="divide-y divide-gray-100">
          {categories.length === 0 ? (
            <div className="p-10 text-center">
              <p className="text-sm font-extrabold text-gray-900">No categories yet</p>
              <p className="mt-1 text-xs text-gray-500">Create a category to start organising your menu.</p>
            </div>
          ) : (
            categories.map((cat: any) => {
              ...
              return (
                <article key={cat.id} className="flex items-center gap-4 p-4">
                  <div className="min-w-0 flex-1">
                    <p className="text-sm font-extrabold text-gray-900">{cat.name}</p>
                    {cat.description && (
  ...
  return (
    <div className="grid gap-5 lg:grid-cols-[0.8fr_1.2fr]">
      <section className="rounded-2xl bg-white p-5 shadow-sm">
        ...
        <h2 className="font-extrabold tracking-tight mt-1 text-2xl">Give guests a reason</h2>
        <div className="mt-5 space-y-3">
          <Input
            value={form.code}
            onChange={(e) =>
            ...
            placeholder="CODE"
            className="h-11 rounded-xl border-gray-200 font-extrabold"
          />
          <Input
          ...
          <div className="flex gap-2">
            <select
              ...
              className="h-11 rounded-xl border border-gray-200 bg-white px-3 text-sm font-bold"
            >
      ...
      <section className="rounded-2xl bg-white p-5 shadow-sm">
        ...
        <h2 className="font-extrabold tracking-tight mt-1 text-2xl">On the counter now</h2>
        <div className="mt-5 space-y-3">
          {(data.offers ?? []).length ? (
      ...
      <div className="flex items-center gap-3">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="h-11 w-full rounded-xl border border-gray-200 bg-white pl-10 pr-4 text-sm outline-none focus:ring-2 focus:ring-[#c84630]"
          />
        </div>
      </div>
...
function RestaurantPanel({
  ...
  return (
    <section className="max-w-3xl rounded-2xl bg-white p-5 shadow-sm">
      ...
      <label className="mt-4 block text-sm font-extrabold text-gray-900">
        Description
        <textarea
          value={form.description}
          onChange={(e) =>
          ...
          className="mt-2 min-h-24 w-full rounded-xl border border-gray-200 bg-white p-3 text-sm outline-none focus:ring-2 focus:ring-[#c84630]"
        />
      </label>
...
function Toggle({
  ...
  return (
    <label className="flex items-center justify-between rounded-xl border border-gray-200 bg-white p-3 text-sm font-extrabold text-gray-900">
      {label}
      <input
        checked={checked}
    ...
    <main className="min-h-screen bg-[#F8F9FA] p-8">
      <div className="mx-auto max-w-7xl animate-pulse space-y-5">
        ...
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
          {[1, 2, 3, 4].map((key) => (
            <div key={key} className="h-40 rounded-2xl bg-gray-100" />
          ))}
        </div>
